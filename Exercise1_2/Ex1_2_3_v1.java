...
public class Ex1_2_3_v1 {
	public static void main(String[] args) {
		int N = Integer.parseInt(args[0]);
		double min = Double.parseDouble(args[1]);
		double max = Double.parseDouble(args[2]);
		StdDraw.setXscale(min, max);//将x的范围设为（min,max）
		StdDraw.setYscale(min, max);//将x的范围设为（min,max）
		Point2D[] leftTopPoints = new Point2D[N];//N个左上顶点
		Point2D[] rightBottomPoints = new Point2D[N];//N个右下顶点
